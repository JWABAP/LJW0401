CLASS lhc_ZI_ONLINE_SHOP_48 DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
...
CLASS lhc_ZI_ONLINE_SHOP_48 IMPLEMENTATION.
...
TRY.
    ...
    zbp_i_online_shop_48=>cv_pr_mapped = ls_pr_mapped.
...
DATA:
      online_shops TYPE TABLE FOR UPDATE ZI_ONLINE_SHOP_48,
      online_shop  TYPE STRUCTURE FOR UPDATE ZI_ONLINE_SHOP_48.

    READ ENTITIES OF ZI_ONLINE_SHOP_48 IN LOCAL MODE
       ENTITY Online_Shop
        ALL FIELDS
    ...
    LOOP AT lt_online_shop_result INTO DATA(online_shop_read).
      ...
      ENDIF.
    ENDLOOP.
   MODIFY ENTITIES OF ZI_ONLINE_SHOP_48 IN LOCAL MODE
    ENTITY ZI_ONLINE_SHOP_48 UPDATE SET FIELDS WITH online_shops
    MAPPED   DATA(ls_mapped_modify)
    FAILED   DATA(lt_failed_modify)
    REPORTED DATA(lt_reported_modify).

    DATA: lt_create_pr_imp TYPE TABLE FOR ACTION IMPORT  ZI_ONLINE_SHOP_48~create_pr,
...
CLASS lsc_ZI_ONLINE_SHOP_48 DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
    METHODS save_modified REDEFINITION.
ENDCLASS.

CLASS lsc_ZI_ONLINE_SHOP_48 IMPLEMENTATION.
  ...
  METHOD save_modified.
    ...
    IF zbp_i_online_shop_48=>cv_pr_mapped-purchaserequisition IS NOT INITIAL.

      LOOP AT zbp_i_online_shop_48=>cv_pr_mapped-purchaserequisition ASSIGNING FIELD-SYMBOL(<fs_pr_mapped>).
